...
class WP_Extend_Toolbar {
	...
	function admin_bar_menu() {
		...
		$title = sprintf(
			'<span class="" style="font-size:13px;">テーマ : </span> <span class="ab-label">%s</span><span class="" style="font-size:13px;">テンプレート : </span> <span class="ab-label">%s</span>',
			get_template(),
			basename( $template )
		);
	...
	function enqueue() {
		...
		wp_enqueue_style( 'wp-extend-toolbar', plugins_url( "css/wp-extend-toolbar.css", __FILE__ ), array(), '1.2.1' );
		wp_enqueue_script( 'wp-extend-toolbar', plugins_url( "js/wp-extend-toolbar.js", __FILE__ ), array( 'jquery' ), '1.2.1', true );
